...
| 日付 | 変更内容 |
|------|----------|
| 2026-03-08 | 週次振り返り機能の要件を再定義（日々の難易度メモ→振り返りリマインド→目標チューニングのフローに変更） |
...
- **達成TODO一覧**:
    ...
    - その日に達成したTODO（目標TODO＋マイルーティン）を箇条書き形式で表示
...
目標レベル（Gold/Silver/Bronze）とは別に、毎日決まった時間に実行したい習慣（例：「07:00 コーヒーを淹れて読書」）を記録するセクションです。

#### 特徴
- 達成度（Bronze/Silver/Gold）の判定には**影響しない**
- ユーザーが「マイルーティン」画面 (`/routine`) で設定した実施中のルーティンが自動的に表示される
- 記録画面内で新しくルーティンを追加したり編集・削除することはできない（操作はマイルーティン画面で行う）

#### チェック動作
- 実行できたルーティンにチェックを入れると、その日の達成として記録される
- 時間帯（HH:MM）の順にソートされて表示される
...
4. マイルーティンの達成状態（チェックの有無）を `daily_todo_records` に登録・更新
...
- **当日中（記録日の23:59:59まで）**: すべての内容を編集可能
  - TODOチェック状態（Recovery/Gold/Silver/Bronze/マイルーティン）
  - 自由記述（Journal）
  - 満足度評価
...
- **表示内容**: その日の記録・日報画面で入力した全ての内容を表示
    ...
    - 達成したTODO一覧（目標TODO＋マイルーティン）
      - 各TODOは達成/未達成の状態で表示
      - Recovery/Gold/Silver/Bronze/マイルーティンのカテゴリごとにグループ化
    - 自由記述（Journal）
...
## 8. 週次振り返り機能（Weekly Review）

Gold/Silver/Bronze目標が、自分の生活リズムやその日の状況に対して適切な難易度に設定されているかを週1回見直し、チューニングするための機能です。
「日々の記録時に感じた目標の難易度に関するメモ」を蓄積し、それを材料にして目標を修正する運用フローを実現します。
...
### 8-A. 日報入力時の難易度メモ（記録画面の拡張）

日報（記録）画面において、その日の目標難易度（特にBronze/Silver）が適切だったかを振り返る入力欄を追加する。

- **入力内容**: 自由記述のテキスト（例：「講義が長引いてSilverがきつかった」「Bronzeはこれなら絶対できる」など）
- **入力タイミング**: 日報保存時。任意入力（空でも記録確定可能）
- **表示位置**: 記録画面の満足度評価の下に配置
- **保存先**: `daily_records` テーブルに `difficulty_memo` カラム（TEXT, NULL許容）を追加

### 8-B. 振り返りリマインド（ホーム画面の拡張）

月曜日以降、ホーム画面に「今週の振り返りをしましょう」というダイアログを表示する。

- **表示開始**: 毎週月曜日から表示を開始する
- **表示終了条件**: 週次振り返りページで「振り返り完了」ボタンを押すまで**毎日出続ける**（火曜・水曜…と押すまで表示され続ける）
- **振り返り完了後**: その週のダイアログは非表示になり、次の月曜日まで再表示されない
- **表示形式**: ホーム画面に目立つダイアログ
- **アクション**: クリックすると週次振り返りページへ遷移
- **管理テーブル**: `weekly_review_status` テーブルで、週ごとの振り返り完了ステータスを管理（`week_start_date`, `completed_at`）

### 8-C. 週次振り返りページ（チューニング画面）

#### アクセス方法
- サイドバーのメニュー項目「週次振り返り」から遷移
- ホーム画面の振り返りリマインドダイアログから遷移
- メニュー順序: ホーム → 記録 → カレンダー → 週次振り返り → 目標変遷 → 設定

#### 対象期間
- 直近1週間のデータを中心に振り返る

#### セクション1: 今週の難易度メモ一覧
- 1週間で蓄積された「日々の難易度メモ」を日付順に一覧表示する
- ユーザーはこれを見ながら、今の自分のフェーズ（長期休み中、授業期間中など）にSilver/Bronze目標が合っているかを考える
- メモがない日は「メモなし」と表示

#### セクション2: 目標のチューニング（編集）
- メモを参照しながら、画面上で直接Todoの内容を編集・削除・追加できる
- 各レベル最低1件のTodoは残す
- 分析データではなく、ユーザー自身の感覚をもとに最適な難易度に調整することを主眼とする
- 変更時は変更前の内容が `goal_level_history` に記録される

**編集権限（レベル別）:**
- **Gold目標**: 制約なし。週次振り返りページからいつでも編集可能
- **Bronze/Silver目標**: **月曜日のみ**編集可能。火曜日以降は読み取り専用で表示される
  - 月曜日に週次振り返りページを開いた場合: Bronze/Silverの編集フォームが有効
  - 火曜日以降に開いた場合: Bronze/Silverは読み取り専用で表示し、「Bronze/Silver目標の変更は毎週月曜日に行えます」と説明文を表示
  - Goldは曜日に関係なく常に編集可能

#### セクション3: Gold目標の妥当性チェック（アラート）
- 「Gold目標が努力目標として正しく機能しているか」をシステムがチェックし、該当する場合はアラートを表示する
  - **チェック1: 達成不可能な目標の検出**: 過去N週間連続で一度も達成されていないTodoがある場合に警告（例：「このTodoは3週間達成されていません。目標を見直しますか？」）
  - **チェック2: Gold達成時の満足度乖離**: Goldを達成しているのに満足度が4未満の日が多い場合に指摘（例：「Gold達成日の満足度が低めです。Gold目標の内容が本当にやりたいこととズレていませんか？」）
- アラートを見て、ユーザーはGold目標の内容をセクション2で見直す
