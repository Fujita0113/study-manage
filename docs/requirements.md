...
**レベル変更期間の表示:**
- 目標のレベルアップ・ダウンが発生した期間を、横軸上に`<->`のような区間として表示
- 区間の上部に、その期間の評価ラベルを表示:
  - **Goldレベルアップ**: 「絶好調！」
  - **Silverレベルアップ**: 「そこそこの調子！」
  - **Bronzeレベルアップ**: 「習慣継続中！」
  - **レベルダウン**: 「不調気味...」
- 複数の期間が同時に表示される場合は、ラベルが重ならないよう上下交互に配置

**縦線（レベル変更の区切り線）:**
- 目標のレベルが変更された日付に、グラフ全体を貫通する縦線を引く
...
- **横軸**: 日付（折れ線グラフと同じ横軸）
- **各棒の構成**: 日ごとにセグメント分割され、その日の目標レベル(Lv.1, Lv.2など)に応じて色分けされる
  - 例：「Lv.2 | Lv.1 | Lv.1 | Lv.2 | ...」のように日ごとに区切られる
  - レベルが高いほど濃い色で表示（グラデーション）
...
**縦線（レベル変更の区切り線）:**
- 折れ線グラフと同じく、レベル変更時に棒グラフ全体を貫通する縦線を引く
- 縦線により、どの期間でどのレベルの目標だったかが視覚的にわかる
...
- **データ取得方法**: 各日の目標レベルと内容を、`goal_level_history`テーブルから期間指定で取得
- **色分けロジック**: レベル（Lv.1, Lv.2...）が高いほど濃い色で表示
