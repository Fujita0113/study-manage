...
| 日付 | 変更内容 |
|------|----------|
| 2026-03-08 | 週次振り返りページ、目標変更メモ、目標変更履歴の追加 |
...
## 8. 週次振り返り画面（Weekly Review）

週単位でGold/Silver/Bronzeの達成状況を振り返り、目標の調整判断に使う画面です。
...
- 今週新たに追加したTodoには「← 今週追加」ラベルを付与
- **「仮外し計算」**: 各Todoを外した場合の推定Gold達成率と増減幅を表示

#### セクション3: Gold目標の編集
- **編集権限**: 制約なし。いつでも編集可能
- 既存Todoの編集・削除、新規追加が可能（最低1件は残す）
- 即時反映され、その日以降の日報から新しいGold目標が適用される
- 変更前の内容は `goal_level_history` に記録される

#### セクション4: Silver/Bronze目標の編集
- **編集権限**: 「月曜日以降に初めて週次振り返りページを開いた日」の当日中（23:59:59まで）のみ編集可能
- 既存Todoの編集・削除、新規追加が可能（最低1件は残す）
- **変更理由の入力**: 編集後、「変更理由」の入力が必須（自由記述、1文字以上）
- 変更理由や記録は `goal_change_log` テーブルに格納される
- 編集不可時は読み取り専用として表示し、「月曜日以降に初めてこのページを開いた日に編集できます」と説明文を表示

#### セクション5: 変更案メモ
- 週の途中で「こうしたほうが良いかも」と思ったことをその場で書き留めるメモ機能
- 今週分と過去3週分のメモを表示（過去分は読み取り専用）
- 1件ずつメモを追加するリスト形式
